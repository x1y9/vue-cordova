...
function finalize () {
  ...
  if (env.android) {
    let commitNo = spawn.getOutput('git',['rev-parse', '--short', 'HEAD']).stdout.trim()
    ...
    fs.renameSync(path.resolve(__dirname, sourceApk), path.resolve(__dirname, '../' + targetApk))  
  ...
  if (env.ios) {
    ...
    try {
      ...
      fs.renameSync(configFile + '.temp', configFile)
    }
  }
}
